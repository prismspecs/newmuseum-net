...
$(document).ready(function(){
    ...
    var initBackToMobileCookie = function(){
        var btn = document.getElementById('view-mobile-button');
...
function initSpacesLanding(){
    ...
    if($('.controller-spaces.action-landing').size()){
        ...
        $.get('/spaces/refresh/', null, function(data){
...
function externalLinks(){
    $('a[target="_blank"][href!="http://kettlenyc.com"][href^=http][data-external!=false]', $('#header, #top, #footer')).each(function(index){
...
function calendarFilter(){
    ...
    */
    ...
    $('.dd-container .next').click(function(e) {
        ...
        var newOffsetStr;
        var newOffset = parseInt(offset, 10) + 2;
        if (newOffset >= 0) {
            newOffsetStr = newOffset.toString();
        }
        else {
            newOffsetStr =  'n' + (0 - newOffset).toString();
        }
        $.get('/events/calendar/' + newOffsetStr, update);
        return false;
    });
...
function initTabModule(){
    ...
    if($('.tab-module').size()){
        ...
        $('.tab-module > ul > li:not(.exclude) > a').click(function(e){
            e.preventDefault();

            if(!animating){
                // url hash change deep-linking
                let newUrl;
                ...
                // animate tab visibility
                animating = true;
                var index = $('.tab-module li:not(.exclude)').index($(this).parent());
        ...
        if (location.hash) {
            ...
            $('.tab-module a[href="#'+hash[1]+'"]').click();
...
function initInView(){
    // disable on support page
    if($('.action-view.pass-support').size()) {
        return false;
    }
...
function getTopBarOffset(){
    return $('#top').outerHeight(true) + $('#header').outerHeight(true) + $('#hero').height() - 50;
}
